  ...
  return (
    <Screen>
      ...
      <Body>
        ...
        <div className="relative mt-6 rounded-[18px] border border-chalk/6 bg-surface py-7">
          <Jeans denim={colour.denim} fit={product.fit} className="mx-auto h-60" />
          {fit.size && (
            <span className="absolute left-3.5 top-3.5 rounded-md bg-chalk px-2.5 py-1
                             text-[11px] font-medium text-ground">
              Your size · <span className="figure font-semibold">{fit.size}</span>
            </span>
          )}
        </div>

        <p className="figure pt-5 text-[9.5px] font-medium tracking-[.14em] text-faint uppercase">{product.brand}</p>
        <h1 className="pt-1 font-serif text-[27px] leading-[1.1] font-normal">
          ...
          {product.colours.map((c) => (
            <button key={c.id} type="button" onClick={() => setColour(c)}
                    ...
                    className={`h-7 w-7 rounded-full transition
                      ${colour.id === c.id ? "ring-2 ring-chalk ring-offset-2 ring-offset-ground" : ""}`}
        ...
        <div className="mt-5 rounded-[18px] border border-chalk/8 bg-surface px-6 py-6 text-chalk">
          {fit.size ? (
            ...
            <>
              <p className="font-serif text-[21px] leading-[1.2]">{fit.headline}</p>
              <p className="pt-2 text-[12.5px] leading-[1.5] text-mute">
          ...
          {why && (
            <div className="mt-4 space-y-1.5 border-t border-chalk/14 pt-4
              ...
              <p>
                Decided from your{" "}
                <span className="figure text-chalk">
